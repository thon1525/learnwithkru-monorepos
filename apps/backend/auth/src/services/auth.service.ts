...
import { HandleUser } from '@auth/utils/handleUser';
export class AuthServices {
  ...
  async Login(
    ...
    try {
      ...
      let DataUsers: UserPersional | undefined | StudentType;
      const requestUser = new RequestUserService();
      let Datastuden: StudentType | undefined;
      switch (existingUser.role) {
        ...
        case 'STUDENT':
          ...
          logger.info(`Data ${existingUser.id}`);
          const requestStudent = new HttpReqStudent();
          const { DataStuent } = await requestStudent.GetStudentByAuthId(
            existingUser.id
          );
          logger.info(`DATA: ID ${DataStuent.email}`);
          DataUsers = {
            firstname: DataStuent.firstname,
            lastname: DataStuent.lastname,
            email: DataStuent.email,
            picture: DataStuent.picture,
            role: DataStuent.role,
            schoolName: DataStuent.schoolName,
            education: DataStuent.education,
            grade: DataStuent.grade,
            studentCard: DataStuent.studentCard,
            _id: DataStuent._id,
          };
          break;
        case 'TUTOR':
  ...
  async Logout(decodedUser: DecodedUser): Promise<boolean> {
    ...
    try {
      // Log the role and user id for debugging
      logger.info(`Logging out user with role: ${role}, ID: ${id}`);
      const handleUser = new HandleUser();
      const DataUsers = await handleUser.getUserDataByRole(id, role);
      ...
      logger.info(`User successfully logged out: ${id}`);
      return true;
    } catch (error: unknown) {
      logger.error(
        `Error during logout for user ID: ${id}, Role: ${role}`,
        error
      );
      throw new ApiError(
